...
class GRU4Rec:
    ...
    def model(self, X, H, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        if self.embedding:
            ...
            #tmp=T.extra_ops.to_one_hot(X,self.n_items)
            #pp(tmp)
            #Sx = T.dot(tmp, self.Wx[0])
            #vec = Sx + self.Bh[0]
            Sx = self.Wx[0][X]
            vec = Sx + self.Bh[0]
    ...
    def predict_next_batch(self, session_ids, input_item_ids, predict_for_item_ids=None, batch=16):
        '''
