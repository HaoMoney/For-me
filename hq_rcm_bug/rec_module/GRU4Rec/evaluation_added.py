...
def evaluate_sessions_batch(pr, test_data, items=None, cut_off=2, batch_size=16, break_ties=False, session_key='SessionId', item_key='ItemId', active_key='Active', time_key='Time'):
    '''
    ...
    in_acx = np.zeros(batch_size,dtype=np.float64)
    #print in_idx
    #print in_idx.dtype
    ...
    while True:
        ...
        in_acx[valid_mask] = test_data.Active.values[start_valid]
        #print in_idx
        for i in range(minlen-1):
            out_idx = test_data.ItemId.values[start_valid+i+1]
            out_acx = test_data.Active.values[start_valid+i+1]
            if sampled_items:
                ...
                preds = pr.predict_next_batch(iters, in_idx, in_acx, None, batch_size) #TODO: Handling sampling?
