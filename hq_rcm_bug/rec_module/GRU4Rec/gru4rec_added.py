...
class GRU4Rec:
    ...
    def __init__(self, loss, final_act, hidden_act, layers,
                 ...
                 adapt='adagrad', decay=0.9, grad_cap=0,
                 sigma=0, init_as_normal=False, reset_after_session=True, train_random_order=False, time_sort=True,
                 session_key='SessionId', item_key='ItemId', active_key='Active',time_key='Time'):
        self.layers = layers
    ...
    def adadelta(self, param, grad, updates, sample_idx = None, epsilon = 1e-6):
        ...
        gradient_scaling = T.cast(T.sqrttheano(acc_new + epsilon), theano.config.floatX)
        return grad / gradient_scaling
    ...
    def RMSprop(self, cost, params, full_params, sampled_params, sidxs, epsilon=1e-6):
        ...
        return updates
    def model(self, idX, actX, H, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        if self.embedding:
            Sx = self.E[idX]
            cat_Sx = T.concatenate([Sx,actX],axis=1)
            y = self.dropout(cat_Sx, drop_p_embed)
            H_new = []
            start = 0
        else:
            Sx = self.Wx[0][X]
    ...
    def fit(self, data, retrain=False, sample_store=10000000):
        ...
        acX = T.dmatrix()
        Y = T.ivector()
        H_new, Y_pred, sampled_params = self.model(idX, acX, self.H, Y, self.dropout_p_hidden, self.dropout_p_embed)
        ...
        train_function = function(inputs=[idX,acX, Y], outputs=cost, updates=updates, allow_input_downcast=True)
        ...
        for epoch in range(self.n_epochs):
            ...
            while not finished:
                ...
                out_act = data_act[start].reshape(len(data_act[start]),1)
                #print len(out_idx)
                #print len(out_act)
                for i in range(minlen-1):
                    ...
                    out_act = data_act[start+i+1].reshape(len(data_act[start+i+1]),1)
                    if self.n_sample:
                        if sample_store:
                        ...
                        y = out_idx
                    cost = train_function(in_idx, in_act, y)
                    c.append(cost)
                    if np.isnan(cost):
    ...
    def predict_next_batch(self, session_ids, input_item_ids, input_acs, predict_for_item_ids=None, batch=16):
        '''
        ...
        if self.predict is None or self.predict_batch!=batch:
            ...
            if predict_for_item_ids is not None:
                H_new, yhat, _ = self.model(idX, acX, self.H, Y, predict=True)
            else:
                H_new, yhat, _ = self.model(idX, acX, self.H, predict=True)
            updatesH = OrderedDict()
            for i in range(len(self.H)):
                updatesH[self.H[i]] = H_new[i]
            if predict_for_item_ids is not None:
                self.predict = function(inputs=[idX, acX, Y], outputs=yhat, updates=updatesH, allow_input_downcast=True)
            else:
                self.predict = function(inputs=[idX, acX], outputs=yhat, updates=updatesH, allow_input_downcast=True)
        ...
        in_acxs = input_acs.reshape(len(input_acs),1)
        #print in_idxs
        #print len(in_idxs)
        if predict_for_item_ids is not None:
            ...
            preds = np.asarray(self.predict(in_idxs,in_acxs)).T
            #print "------------------------------------------------"
            #print pd.DataFrame(data=preds, index=self.itemidmap.index)
