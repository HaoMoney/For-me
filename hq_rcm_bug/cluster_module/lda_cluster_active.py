...
data = pd.read_csv(PATH_TO_ORIGINAL_DATA + 'cleared_eclipse',sep='\t')
...
with open('../rec_module/report_labels_train_eclipse','w') as f:
    f.write('SessionId\tItemId\tActive\tTime\n')
    for key in train_report_dict:
        for item in train_report_dict[key]:
            active=ass_dict[item[1]][0]
            #repair_time=ass_dict[item[1]][1]
            #skill=float(ass_dict[item[1]][2])
            f.write(str(key)+'\t'+item[1]+'\t'+str(active/total)+'\t'+item[0])
            f.write('\n')
test_report_dict={}
...
with open('../rec_module/report_labels_test_eclipse','w') as f:
    f.write('SessionId\tItemId\tActive\tTime\n')
    for key in test_report_dict:
